...
IPAddress::IPAddress(const IPType ip_type) {
  _type = ip_type;
  memset(_address.bytes, 0, sizeof(_address.bytes));
}

IPAddress::IPAddress(const std::uint8_t first_octet, const std::uint8_t second_octet, const std::uint8_t third_octet,
                     const std::uint8_t fourth_octet) {
  _type = IPv4;
  memset(_address.bytes, 0, sizeof(_address.bytes));
...
IPAddress::IPAddress(std::uint8_t o1, std::uint8_t o2, std::uint8_t o3, std::uint8_t o4, std::uint8_t o5,
                     std::uint8_t o6, std::uint8_t o7, std::uint8_t o8, std::uint8_t o9, std::uint8_t o10,
                     std::uint8_t o11, std::uint8_t o12, std::uint8_t o13, std::uint8_t o14, std::uint8_t o15,
                     std::uint8_t o16) {
  _type              = IPv6;
  _address.bytes[0]  = o1;
  _address.bytes[1]  = o2;
...
IPAddress::IPAddress(const std::uint32_t address) {
  // IPv4 only
  _type = IPv4;
  memset(_address.bytes, 0, sizeof(_address.bytes));
...
IPAddress::IPAddress(const std::uint8_t* address) : IPAddress(IPv4, address) { }

IPAddress::IPAddress(const IPType ip_type, const std::uint8_t* address) {
  _type = ip_type;
  if (ip_type == IPv4) {
    memset(_address.bytes, 0, sizeof(_address.bytes));
    memcpy(&_address.bytes[IPADDRESS_V4_BYTES_INDEX], address, sizeof(std::uint32_t));
  } else {
...
std::uint8_t IPAddress::operator[](const int index) const {
  if (_type == IPv4) {
...
std::uint8_t& IPAddress::operator[](const int index) {
  if (_type == IPv4) {
    return _address.bytes[IPADDRESS_V4_BYTES_INDEX + index];
  }
  return _address.bytes[index];
...
IPAddress& IPAddress::operator=(const std::uint8_t* address) {
...
IPAddress& IPAddress::operator=(const std::uint32_t address) {
  // IPv4 conversion
...
bool IPAddress::fromString6(const char* address) {
  ...
  while (*address) {
    if (char c = tolower(*address++); isalnum(c) && c <= 'f') {
      if (c >= 'a')
        c -= 'a' - '0' - 10;
      acc = acc * 16 + (c - '0');
      if (acc > 0xffff)
...
String IPAddress::toString4() const {
  char szRet[16];
  snprintf(szRet, sizeof(szRet), "%u.%u.%u.%u", _address.bytes[IPADDRESS_V4_BYTES_INDEX],
           _address.bytes[IPADDRESS_V4_BYTES_INDEX + 1], _address.bytes[IPADDRESS_V4_BYTES_INDEX + 2],
           _address.bytes[IPADDRESS_V4_BYTES_INDEX + 3]);
  return String(szRet);
}
...
String IPAddress::toString6() const {
  ...
  snprintf(szRet, sizeof(szRet), "%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x",
           _address.bytes[0], _address.bytes[1], _address.bytes[2], _address.bytes[3], _address.bytes[4],
           _address.bytes[5], _address.bytes[6], _address.bytes[7], _address.bytes[8], _address.bytes[9],
           _address.bytes[10], _address.bytes[11], _address.bytes[12], _address.bytes[13], _address.bytes[14],
           _address.bytes[15]);
  return String(szRet);
}
